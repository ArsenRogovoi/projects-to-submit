...
import {
  ARROW_ICON,
  CITIES_INPUT,
  CLICKED_CITIES_CONTAINER,
  DROP_DAWN_CONTAINER,
} from "./domService.js";

export const renderDropDown = (
  citiesToRender: string[],
  ...
  citiesToRender.forEach((city, ind, arr) => {
    ...
    if (ind === 0) divElem.classList.add("rounded-top-3");
    if (ind === arr.length - 1) {
...
export const findMatchingCities = () => {
  ...
  cities.forEach((cityObj) => {
    ...
    inputCharacters.forEach((char) => {
      if (
        cityObj.city.includes(char) === false &&
...
const listItemWasMouseEntered = (divElem: HTMLDivElement) => {
  divElem.classList.add("bg-primary");
};

const listItemWasMouseLeaved = (divElem: HTMLDivElement) => {
  divElem.classList.remove("bg-primary");
};

const listItemWasClicked = (divElem: HTMLDivElement) => {
